import Link from "next/link";

const Sidebar = () => {
  return (
    <>
      <aside className="hidden md:flex md:justify-between flex-col bg-white w-[6.3rem] border-r md:mx-6 md:my-6 rounded-l-3xl overflow-y-scroll text-center">
        <div className="flex items-center justify-center my-6 ">
          <h1 className="text-lg font-bold">
            <Link href="#" className="hover:text-green-500">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="50"
                height="50"
                viewBox="0 0 50 50"
              >
                <g transform="translate(4698 16470)">
                  <rect
                    width="50"
                    height="50"
                    rx="10"
                    transform="translate(-4698 -16470)"
                    fill="#1fa45b"
                  />
                  <g transform="translate(-4688.479 -16455.84)">
                    <path
                      d="M2.248,8.768v2.657c0,.715.817,1.839.817-2.452V2.228a1.124,1.124,0,1,1,2.248,0V8.563a.41.41,0,0,0,.817,0V.9A1.124,1.124,0,1,1,8.379.9V8.461c0,.511.715.613.715,0V1.819a1.175,1.175,0,0,1,2.35,0V9.381c0,.511.715.613.715,0V4.067a1.175,1.175,0,0,1,2.35,0V16.125C14.51,23.482,0,23.686,0,13.877V8.768A1.124,1.124,0,1,1,2.248,8.768Z"
                      transform="translate(0 0.25)"
                      fill="#fff"
                      fillRule="evenodd"
                    />
                    <path
                      d="M173.16,8.768v2.657c0,.715-.715,1.839-.715-2.452V2.228a1.124,1.124,0,1,0-2.248,0V8.563a.41.41,0,0,1-.817,0V.9a1.124,1.124,0,1,0-2.248,0V8.461a.409.409,0,1,1-.817,0V1.819a1.124,1.124,0,1,0-2.248,0V9.381a.409.409,0,1,1-.817,0V4.067a1.124,1.124,0,1,0-2.248,0V16.125c0,7.357,14.51,7.561,14.51-2.248V8.768A1.175,1.175,0,0,0,173.16,8.768Z"
                      transform="translate(-144.549 0.25)"
                      fill="#fff"
                      fillRule="evenodd"
                    />
                  </g>
                </g>
              </svg>
            </Link>
          </h1>
        </div>
        <nav className="p-4 flex items-center justify-center my-9">
          <ul className="space-y-6 mx-auto ">
            <li className="bg-[#E8F0F5] w-10 h-10 rounded-full flex justify-center items-center">
              <a href="#" className="text-gray-600 hover:text-green-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <g clipPath="url(#clip0_151_16608)">
                    <path
                      d="M19.2675 7.55531L12.9467 1.23365C12.1644 0.453638 11.1047 0.015625 10 0.015625C8.89529 0.015625 7.83564 0.453638 7.05334 1.23365L0.73251 7.55531C0.499531 7.7868 0.314818 8.06223 0.189079 8.36563C0.0633394 8.66904 -0.000925334 8.99439 1.00662e-05 9.32281V17.5036C1.00662e-05 18.1667 0.263402 18.8026 0.732243 19.2714C1.20108 19.7403 1.83697 20.0036 2.50001 20.0036H17.5C18.1631 20.0036 18.7989 19.7403 19.2678 19.2714C19.7366 18.8026 20 18.1667 20 17.5036V9.32281C20.0009 8.99439 19.9367 8.66904 19.8109 8.36563C19.6852 8.06223 19.5005 7.7868 19.2675 7.55531ZM12.5 18.337H7.50001V15.0586C7.50001 14.3956 7.7634 13.7597 8.23224 13.2909C8.70109 12.822 9.33697 12.5586 10 12.5586C10.6631 12.5586 11.2989 12.822 11.7678 13.2909C12.2366 13.7597 12.5 14.3956 12.5 15.0586V18.337ZM18.3333 17.5036C18.3333 17.7247 18.2455 17.9366 18.0893 18.0929C17.933 18.2492 17.721 18.337 17.5 18.337H14.1667V15.0586C14.1667 13.9536 13.7277 12.8938 12.9463 12.1124C12.1649 11.331 11.1051 10.892 10 10.892C8.89494 10.892 7.83513 11.331 7.05373 12.1124C6.27233 12.8938 5.83334 13.9536 5.83334 15.0586V18.337H2.50001C2.279 18.337 2.06703 18.2492 1.91075 18.0929C1.75447 17.9366 1.66668 17.7247 1.66668 17.5036V9.32281C1.66745 9.10196 1.75517 8.8903 1.91084 8.73365L8.23168 2.41448C8.70143 1.94692 9.33723 1.68443 10 1.68443C10.6628 1.68443 11.2986 1.94692 11.7683 2.41448L18.0892 8.73615C18.2442 8.89219 18.3319 9.10283 18.3333 9.32281V17.5036Z"
                      fill="#868686"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_151_16608">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li className="bg-[#E8F0F5] w-10 h-10 rounded-full flex justify-center items-center">
              <a href="#" className="text-gray-600 hover:text-green-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <g clipPath="url(#clip0_151_16608)">
                    <path
                      d="M19.2675 7.55531L12.9467 1.23365C12.1644 0.453638 11.1047 0.015625 10 0.015625C8.89529 0.015625 7.83564 0.453638 7.05334 1.23365L0.73251 7.55531C0.499531 7.7868 0.314818 8.06223 0.189079 8.36563C0.0633394 8.66904 -0.000925334 8.99439 1.00662e-05 9.32281V17.5036C1.00662e-05 18.1667 0.263402 18.8026 0.732243 19.2714C1.20108 19.7403 1.83697 20.0036 2.50001 20.0036H17.5C18.1631 20.0036 18.7989 19.7403 19.2678 19.2714C19.7366 18.8026 20 18.1667 20 17.5036V9.32281C20.0009 8.99439 19.9367 8.66904 19.8109 8.36563C19.6852 8.06223 19.5005 7.7868 19.2675 7.55531ZM12.5 18.337H7.50001V15.0586C7.50001 14.3956 7.7634 13.7597 8.23224 13.2909C8.70109 12.822 9.33697 12.5586 10 12.5586C10.6631 12.5586 11.2989 12.822 11.7678 13.2909C12.2366 13.7597 12.5 14.3956 12.5 15.0586V18.337ZM18.3333 17.5036C18.3333 17.7247 18.2455 17.9366 18.0893 18.0929C17.933 18.2492 17.721 18.337 17.5 18.337H14.1667V15.0586C14.1667 13.9536 13.7277 12.8938 12.9463 12.1124C12.1649 11.331 11.1051 10.892 10 10.892C8.89494 10.892 7.83513 11.331 7.05373 12.1124C6.27233 12.8938 5.83334 13.9536 5.83334 15.0586V18.337H2.50001C2.279 18.337 2.06703 18.2492 1.91075 18.0929C1.75447 17.9366 1.66668 17.7247 1.66668 17.5036V9.32281C1.66745 9.10196 1.75517 8.8903 1.91084 8.73365L8.23168 2.41448C8.70143 1.94692 9.33723 1.68443 10 1.68443C10.6628 1.68443 11.2986 1.94692 11.7683 2.41448L18.0892 8.73615C18.2442 8.89219 18.3319 9.10283 18.3333 9.32281V17.5036Z"
                      fill="#868686"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_151_16608">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li className="bg-[#E8F0F5] w-10 h-10 rounded-full flex justify-center items-center">
              <a href="#" className="text-gray-600 hover:text-green-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <g clipPath="url(#clip0_151_16608)">
                    <path
                      d="M19.2675 7.55531L12.9467 1.23365C12.1644 0.453638 11.1047 0.015625 10 0.015625C8.89529 0.015625 7.83564 0.453638 7.05334 1.23365L0.73251 7.55531C0.499531 7.7868 0.314818 8.06223 0.189079 8.36563C0.0633394 8.66904 -0.000925334 8.99439 1.00662e-05 9.32281V17.5036C1.00662e-05 18.1667 0.263402 18.8026 0.732243 19.2714C1.20108 19.7403 1.83697 20.0036 2.50001 20.0036H17.5C18.1631 20.0036 18.7989 19.7403 19.2678 19.2714C19.7366 18.8026 20 18.1667 20 17.5036V9.32281C20.0009 8.99439 19.9367 8.66904 19.8109 8.36563C19.6852 8.06223 19.5005 7.7868 19.2675 7.55531ZM12.5 18.337H7.50001V15.0586C7.50001 14.3956 7.7634 13.7597 8.23224 13.2909C8.70109 12.822 9.33697 12.5586 10 12.5586C10.6631 12.5586 11.2989 12.822 11.7678 13.2909C12.2366 13.7597 12.5 14.3956 12.5 15.0586V18.337ZM18.3333 17.5036C18.3333 17.7247 18.2455 17.9366 18.0893 18.0929C17.933 18.2492 17.721 18.337 17.5 18.337H14.1667V15.0586C14.1667 13.9536 13.7277 12.8938 12.9463 12.1124C12.1649 11.331 11.1051 10.892 10 10.892C8.89494 10.892 7.83513 11.331 7.05373 12.1124C6.27233 12.8938 5.83334 13.9536 5.83334 15.0586V18.337H2.50001C2.279 18.337 2.06703 18.2492 1.91075 18.0929C1.75447 17.9366 1.66668 17.7247 1.66668 17.5036V9.32281C1.66745 9.10196 1.75517 8.8903 1.91084 8.73365L8.23168 2.41448C8.70143 1.94692 9.33723 1.68443 10 1.68443C10.6628 1.68443 11.2986 1.94692 11.7683 2.41448L18.0892 8.73615C18.2442 8.89219 18.3319 9.10283 18.3333 9.32281V17.5036Z"
                      fill="#868686"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_151_16608">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li className="bg-[#E8F0F5] w-10 h-10 rounded-full flex justify-center items-center">
              <a href="#" className="text-gray-600 hover:text-green-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <g clipPath="url(#clip0_151_16608)">
                    <path
                      d="M19.2675 7.55531L12.9467 1.23365C12.1644 0.453638 11.1047 0.015625 10 0.015625C8.89529 0.015625 7.83564 0.453638 7.05334 1.23365L0.73251 7.55531C0.499531 7.7868 0.314818 8.06223 0.189079 8.36563C0.0633394 8.66904 -0.000925334 8.99439 1.00662e-05 9.32281V17.5036C1.00662e-05 18.1667 0.263402 18.8026 0.732243 19.2714C1.20108 19.7403 1.83697 20.0036 2.50001 20.0036H17.5C18.1631 20.0036 18.7989 19.7403 19.2678 19.2714C19.7366 18.8026 20 18.1667 20 17.5036V9.32281C20.0009 8.99439 19.9367 8.66904 19.8109 8.36563C19.6852 8.06223 19.5005 7.7868 19.2675 7.55531ZM12.5 18.337H7.50001V15.0586C7.50001 14.3956 7.7634 13.7597 8.23224 13.2909C8.70109 12.822 9.33697 12.5586 10 12.5586C10.6631 12.5586 11.2989 12.822 11.7678 13.2909C12.2366 13.7597 12.5 14.3956 12.5 15.0586V18.337ZM18.3333 17.5036C18.3333 17.7247 18.2455 17.9366 18.0893 18.0929C17.933 18.2492 17.721 18.337 17.5 18.337H14.1667V15.0586C14.1667 13.9536 13.7277 12.8938 12.9463 12.1124C12.1649 11.331 11.1051 10.892 10 10.892C8.89494 10.892 7.83513 11.331 7.05373 12.1124C6.27233 12.8938 5.83334 13.9536 5.83334 15.0586V18.337H2.50001C2.279 18.337 2.06703 18.2492 1.91075 18.0929C1.75447 17.9366 1.66668 17.7247 1.66668 17.5036V9.32281C1.66745 9.10196 1.75517 8.8903 1.91084 8.73365L8.23168 2.41448C8.70143 1.94692 9.33723 1.68443 10 1.68443C10.6628 1.68443 11.2986 1.94692 11.7683 2.41448L18.0892 8.73615C18.2442 8.89219 18.3319 9.10283 18.3333 9.32281V17.5036Z"
                      fill="#868686"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_151_16608">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li className="bg-[#E8F0F5] w-10 h-10 rounded-full flex justify-center items-center">
              <a href="#" className="text-gray-600 hover:text-green-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <g clipPath="url(#clip0_151_16608)">
                    <path
                      d="M19.2675 7.55531L12.9467 1.23365C12.1644 0.453638 11.1047 0.015625 10 0.015625C8.89529 0.015625 7.83564 0.453638 7.05334 1.23365L0.73251 7.55531C0.499531 7.7868 0.314818 8.06223 0.189079 8.36563C0.0633394 8.66904 -0.000925334 8.99439 1.00662e-05 9.32281V17.5036C1.00662e-05 18.1667 0.263402 18.8026 0.732243 19.2714C1.20108 19.7403 1.83697 20.0036 2.50001 20.0036H17.5C18.1631 20.0036 18.7989 19.7403 19.2678 19.2714C19.7366 18.8026 20 18.1667 20 17.5036V9.32281C20.0009 8.99439 19.9367 8.66904 19.8109 8.36563C19.6852 8.06223 19.5005 7.7868 19.2675 7.55531ZM12.5 18.337H7.50001V15.0586C7.50001 14.3956 7.7634 13.7597 8.23224 13.2909C8.70109 12.822 9.33697 12.5586 10 12.5586C10.6631 12.5586 11.2989 12.822 11.7678 13.2909C12.2366 13.7597 12.5 14.3956 12.5 15.0586V18.337ZM18.3333 17.5036C18.3333 17.7247 18.2455 17.9366 18.0893 18.0929C17.933 18.2492 17.721 18.337 17.5 18.337H14.1667V15.0586C14.1667 13.9536 13.7277 12.8938 12.9463 12.1124C12.1649 11.331 11.1051 10.892 10 10.892C8.89494 10.892 7.83513 11.331 7.05373 12.1124C6.27233 12.8938 5.83334 13.9536 5.83334 15.0586V18.337H2.50001C2.279 18.337 2.06703 18.2492 1.91075 18.0929C1.75447 17.9366 1.66668 17.7247 1.66668 17.5036V9.32281C1.66745 9.10196 1.75517 8.8903 1.91084 8.73365L8.23168 2.41448C8.70143 1.94692 9.33723 1.68443 10 1.68443C10.6628 1.68443 11.2986 1.94692 11.7683 2.41448L18.0892 8.73615C18.2442 8.89219 18.3319 9.10283 18.3333 9.32281V17.5036Z"
                      fill="#868686"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_151_16608">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li className="bg-[#E8F0F5] w-10 h-10 rounded-full flex justify-center items-center">
              <a href="#" className="text-gray-600 hover:text-green-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <g clipPath="url(#clip0_151_16608)">
                    <path
                      d="M19.2675 7.55531L12.9467 1.23365C12.1644 0.453638 11.1047 0.015625 10 0.015625C8.89529 0.015625 7.83564 0.453638 7.05334 1.23365L0.73251 7.55531C0.499531 7.7868 0.314818 8.06223 0.189079 8.36563C0.0633394 8.66904 -0.000925334 8.99439 1.00662e-05 9.32281V17.5036C1.00662e-05 18.1667 0.263402 18.8026 0.732243 19.2714C1.20108 19.7403 1.83697 20.0036 2.50001 20.0036H17.5C18.1631 20.0036 18.7989 19.7403 19.2678 19.2714C19.7366 18.8026 20 18.1667 20 17.5036V9.32281C20.0009 8.99439 19.9367 8.66904 19.8109 8.36563C19.6852 8.06223 19.5005 7.7868 19.2675 7.55531ZM12.5 18.337H7.50001V15.0586C7.50001 14.3956 7.7634 13.7597 8.23224 13.2909C8.70109 12.822 9.33697 12.5586 10 12.5586C10.6631 12.5586 11.2989 12.822 11.7678 13.2909C12.2366 13.7597 12.5 14.3956 12.5 15.0586V18.337ZM18.3333 17.5036C18.3333 17.7247 18.2455 17.9366 18.0893 18.0929C17.933 18.2492 17.721 18.337 17.5 18.337H14.1667V15.0586C14.1667 13.9536 13.7277 12.8938 12.9463 12.1124C12.1649 11.331 11.1051 10.892 10 10.892C8.89494 10.892 7.83513 11.331 7.05373 12.1124C6.27233 12.8938 5.83334 13.9536 5.83334 15.0586V18.337H2.50001C2.279 18.337 2.06703 18.2492 1.91075 18.0929C1.75447 17.9366 1.66668 17.7247 1.66668 17.5036V9.32281C1.66745 9.10196 1.75517 8.8903 1.91084 8.73365L8.23168 2.41448C8.70143 1.94692 9.33723 1.68443 10 1.68443C10.6628 1.68443 11.2986 1.94692 11.7683 2.41448L18.0892 8.73615C18.2442 8.89219 18.3319 9.10283 18.3333 9.32281V17.5036Z"
                      fill="#868686"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_151_16608">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li className="bg-[#E8F0F5] w-10 h-10 rounded-full flex justify-center items-center">
              <a href="#" className="text-gray-600 hover:text-green-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 20 20"
                  fill="none"
                >
                  <g clipPath="url(#clip0_151_16608)">
                    <path
                      d="M19.2675 7.55531L12.9467 1.23365C12.1644 0.453638 11.1047 0.015625 10 0.015625C8.89529 0.015625 7.83564 0.453638 7.05334 1.23365L0.73251 7.55531C0.499531 7.7868 0.314818 8.06223 0.189079 8.36563C0.0633394 8.66904 -0.000925334 8.99439 1.00662e-05 9.32281V17.5036C1.00662e-05 18.1667 0.263402 18.8026 0.732243 19.2714C1.20108 19.7403 1.83697 20.0036 2.50001 20.0036H17.5C18.1631 20.0036 18.7989 19.7403 19.2678 19.2714C19.7366 18.8026 20 18.1667 20 17.5036V9.32281C20.0009 8.99439 19.9367 8.66904 19.8109 8.36563C19.6852 8.06223 19.5005 7.7868 19.2675 7.55531ZM12.5 18.337H7.50001V15.0586C7.50001 14.3956 7.7634 13.7597 8.23224 13.2909C8.70109 12.822 9.33697 12.5586 10 12.5586C10.6631 12.5586 11.2989 12.822 11.7678 13.2909C12.2366 13.7597 12.5 14.3956 12.5 15.0586V18.337ZM18.3333 17.5036C18.3333 17.7247 18.2455 17.9366 18.0893 18.0929C17.933 18.2492 17.721 18.337 17.5 18.337H14.1667V15.0586C14.1667 13.9536 13.7277 12.8938 12.9463 12.1124C12.1649 11.331 11.1051 10.892 10 10.892C8.89494 10.892 7.83513 11.331 7.05373 12.1124C6.27233 12.8938 5.83334 13.9536 5.83334 15.0586V18.337H2.50001C2.279 18.337 2.06703 18.2492 1.91075 18.0929C1.75447 17.9366 1.66668 17.7247 1.66668 17.5036V9.32281C1.66745 9.10196 1.75517 8.8903 1.91084 8.73365L8.23168 2.41448C8.70143 1.94692 9.33723 1.68443 10 1.68443C10.6628 1.68443 11.2986 1.94692 11.7683 2.41448L18.0892 8.73615C18.2442 8.89219 18.3319 9.10283 18.3333 9.32281V17.5036Z"
                      fill="#868686"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_151_16608">
                      <rect width="20" height="20" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
          </ul>
        </nav>
        <div className="flex items-center justify-center my-6 ">
          <h1 className="text-lg font-bold">
            <Link href="#" className="hover:text-green-500">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="50"
                height="50"
                viewBox="0 0 50 50"
              >
                <g transform="translate(4698 16470)">
                  <rect
                    width="50"
                    height="50"
                    rx="10"
                    transform="translate(-4698 -16470)"
                    fill="#1fa45b"
                  />
                  <g transform="translate(-4688.479 -16455.84)">
                    <path
                      d="M2.248,8.768v2.657c0,.715.817,1.839.817-2.452V2.228a1.124,1.124,0,1,1,2.248,0V8.563a.41.41,0,0,0,.817,0V.9A1.124,1.124,0,1,1,8.379.9V8.461c0,.511.715.613.715,0V1.819a1.175,1.175,0,0,1,2.35,0V9.381c0,.511.715.613.715,0V4.067a1.175,1.175,0,0,1,2.35,0V16.125C14.51,23.482,0,23.686,0,13.877V8.768A1.124,1.124,0,1,1,2.248,8.768Z"
                      transform="translate(0 0.25)"
                      fill="#fff"
                      fillRule="evenodd"
                    />
                    <path
                      d="M173.16,8.768v2.657c0,.715-.715,1.839-.715-2.452V2.228a1.124,1.124,0,1,0-2.248,0V8.563a.41.41,0,0,1-.817,0V.9a1.124,1.124,0,1,0-2.248,0V8.461a.409.409,0,1,1-.817,0V1.819a1.124,1.124,0,1,0-2.248,0V9.381a.409.409,0,1,1-.817,0V4.067a1.124,1.124,0,1,0-2.248,0V16.125c0,7.357,14.51,7.561,14.51-2.248V8.768A1.175,1.175,0,0,0,173.16,8.768Z"
                      transform="translate(-144.549 0.25)"
                      fill="#fff"
                      fillRule="evenodd"
                    />
                  </g>
                </g>
              </svg>
            </Link>
          </h1>
        </div>
      </aside>
    </>
  );
};

export default Sidebar;
